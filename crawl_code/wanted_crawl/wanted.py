...
def get_positions_info(url_positions, url_detail, limit, offset_max):
    ...
    for i in tqdm(range(0, offset_max, limit)):
        ...
        if id_list == False: break
        positions_data += [get_detail(url=url_detail, curr_t=current_time, position_id=id) for id in tqdm(id_list, desc="Fetching details")]
...
def export_json(p_data_list, d_path="./wanted_data"):
    ...
    export_date = curr_date.strftime("%Y-%m-%d_%H%M")
    ...
    df = df[new_order]
    df.to_json(f"{d_path}/wanted_positions_{curr_date.strftime('%Y-%m-%d_%H%M')}.json", 
               orient='records', 
               lines=True, 
               force_ascii=False,
               date_format='iso')
